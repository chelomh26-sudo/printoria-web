...
const UBICACIONES = [
  { value: 'casa',         label: '🏠 Mi casa',      color: 'bg-blue-50 text-blue-700 border-blue-200' },
  { value: 'tienda_cubos', label: '🏪 Tienda Cubos', color: 'bg-purple-50 text-purple-700 border-purple-200' },
];
...
export default function Stock() {
  ...
  const filtered = rows.filter(r => {
    const matchSearch = [r.id, r._producto?.nombre, r._material?.nombre]
  ...
  function save(e) {
    ...
    const data = {
      ...
      gramosUsados: gramosAuto,
      ubicacion: form.ubicacion || 'casa',
    };
    if (isNew) {
        ...
        <form onSubmit={save} className="bg-white rounded-2xl border border-zinc-200 shadow-sm p-6 space-y-5">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input className={inp} value={form.id} disabled={!isNew}
                onChange={e => set('id', e.target.value.toUpperCase())} />
            </div>
            <div>
              <label className={lbl}>Fecha de producción</label>
              <input type="date" className={inp} value={form.fechaProduccion}
                onChange={e => set('fechaProduccion', e.target.value)} />
            </div>
          </div>

          <div className="bg-zinc-50 rounded-xl p-4 space-y-3">
            <p className="text-xs font-bold text-zinc-400 uppercase tracking-wider">📍 ¿Dónde está este stock?</p>
            <div className="grid grid-cols-2 gap-3">
              {UBICACIONES.map(u => (
                <button
                  key={u.value}
                  ...
                  className={[
                    ...
                    form.ubicacion === u.value
                      ? 'border-[#96d629] bg-[#f0fdf4] text-[#4a7c00] shadow-sm scale-[1.02]'
          ...
          <div className="bg-zinc-50 rounded-xl p-4 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input className={inp} value={form.productoId}
                  onChange={e => set('productoId', e.target.value.toUpperCase())} placeholder="P001" />
              </div>
              <div>
                <label className={lbl}>Nombre (auto)</label>
                <input className={ro} readOnly
                  value={producto ? producto.nombre : form.productoId ? '⚠ No encontrado' : ''} />
              </div>
            </div>
            <div>
              <label className={lbl}>Cantidad producida *</label>
              <input type="number" min="1" className={inp} value={form.cantidad}
          ...
          <div className="bg-zinc-50 rounded-xl p-4 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input className={inp} value={form.materialId}
                  onChange={e => set('materialId', e.target.value.toUpperCase())} placeholder="M001" />
              </div>
              <div>
                <label className={lbl}>Material (auto)</label>
                <input className={ro} readOnly
                  value={material ? material.nombre + ' ' + material.marca : form.materialId ? '⚠ No encontrado' : ''} />
              </div>
            </div>
            {producto && (
              <div className="bg-white rounded-lg p-3 text-xs text-zinc-500 space-y-1">
                <div className="flex justify-between">
                  <span>Gramos/pieza:</span>
                  <span className="font-bold text-zinc-700">{fmtN(producto.gramos || 0, 2)}g</span>
                </div>
                <div className="flex justify-between">
                  <span>Gramos total lote ({cantidad} pzas):</span>
                  <span className="font-bold text-zinc-700">{fmtN(gramosAuto, 2)}g</span>
                </div>
                {material && (
                  <>
                    <div className="flex justify-between">
                      <span>Costo/g:</span>
                      <span className="font-bold text-zinc-700">{'$'}{fmtN(cpg, 4)}</span>
                    </div>
                    <div className="flex justify-between border-t border-zinc-100 pt-1">
                      <span className="font-semibold">Costo total lote:</span>
                      <span className="font-bold text-[#96d629]">{fmt(costoTotal)}</span>
                    </div>
                  </>
                )}
              </div>
          ...
          <div className="flex justify-end gap-3 pt-2 border-t border-zinc-200">
            <button type="button" onClick={() => setEditing(null)}
              className="bg-zinc-100 hover:bg-zinc-200 text-zinc-700 px-5 py-2 rounded-lg text-sm font-medium">
              Cancelar
            </button>
            <button type="submit"
              className="bg-[#96d629] hover:bg-[#78b01e] text-black font-bold px-5 py-2 rounded-lg text-sm">
              Guardar
            </button>
          </div>
        </form>
    ...
    <div className="p-5 space-y-5 max-w-4xl mx-auto">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-zinc-500 font-medium">
            {stock.length} entradas{' · '}
            <span className="font-bold" style={{ color: '#96d629' }}>{totalPiezas} piezas totales</span>
          </p>
        </div>
        <button
          onClick={openNew}
          className="bg-[#96d629] hover:bg-[#78b01e] text-black font-bold px-4 py-2 rounded-xl text-sm shadow-sm transition-all active:scale-95"
        >
          + Agregar
        </button>
      </div>
      ...
      {top3.length > 0 && (
        <div className="bg-white rounded-2xl border border-zinc-200 shadow-sm p-4">
          <p className="text-xs font-black text-zinc-400 uppercase tracking-wider mb-3">🏆 Mayor stock</p>
            ...
            {top3.map((s, i) => {
              ...
              return (
                <div key={s.id} className="rounded-xl p-3 text-center"
                  style={{
                    background: i === 0 ? '#f0fdf4' : i === 1 ? '#eff6ff' : '#fefce8',
                    border: '1px solid',
                    borderColor: i === 0 ? '#bbf7d0' : i === 1 ? '#bfdbfe' : '#fde68a',
                  }}>
                  <p className="text-2xl font-black"
                    style={{ color: i === 0 ? '#15803d' : i === 1 ? '#1d4ed8' : '#92400e' }}>
                    {s.cantidad}
                  </p>
                  <p className="text-xs font-bold text-zinc-600 truncate mt-0.5">
                    {s._producto?.nombre || s.productoId}
                  </p>
      ...
      <div className="flex gap-2 flex-wrap">
        <input
          ...
          placeholder="🔍 Buscar por producto, material o ID..."
        />
        <select
          className="bg-zinc-100 border border-zinc-300 rounded-lg px-3 py-2 text-zinc-700 text-sm focus:border-[#96d629] focus:outline-none"
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-12 text-zinc-400">
          ...
          <p className="text-sm">Agrega piezas terminadas con el botón de arriba</p>
        </div>
      ) : (
            ...
            {filtered.map(s => {
              ...
              return (
                <div key={s.id} className="flex items-center gap-3 px-5 py-3 hover:bg-zinc-50 transition-colors">
                  <span className="text-xs font-bold px-2 py-0.5 rounded-full bg-zinc-100 text-zinc-500 flex-shrink-0">
                    {s.id}
                  </span>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={[
                        ...
                        ubic?.color || 'bg-blue-50 text-blue-700 border-blue-200',
                      ].join(' ')}>
                        {ubic?.label || '🏠 Mi casa'}
                      </span>
                    </div>
                    <p className="text-xs text-zinc-500">
                      {s._material ? s._material.nombre + ' ' + s._material.marca : s.materialId}
                      {s.gramosUsados ? ' · ' + fmtN(s.gramosUsados, 1) + 'g' : ''}
                      {' · '}{s.fechaProduccion}
                    </p>
                  </div>
                  ...
                  <div className="flex gap-2 flex-shrink-0">
                    <button onClick={() => openEdit(s)}
                      className="text-xs bg-zinc-100 hover:bg-zinc-200 text-zinc-600 px-2 py-1 rounded-lg font-medium">
                      ✏️
                    </button>
                    <button onClick={() => del(s.id)}
                      className="text-xs bg-red-50 hover:bg-red-100 text-red-500 px-2 py-1 rounded-lg font-medium">
                      🗑
                    </button>
                  </div>
                </div>
