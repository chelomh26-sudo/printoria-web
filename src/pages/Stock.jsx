...
function emptyForm(stock) {
  return { id: getNextId(stock), fechaProduccion: TODAY(), productoId: '', materialId: '', cantidad: 1 };
}

export default function Stock() {
  const { stock, setStock, products, materials } = usePrintoria();
  const [editing, setEditing]   = useState(null);   // null = lista, {} = nuevo/editar
  ...
  const filtered = rows.filter(r =>
    [r.id, r._producto?.nombre, r._material?.nombre].some(v => v?.toLowerCase().includes(search.toLowerCase()))
  );
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-zinc-500 font-medium">
            {stock.length} entradas · <span className="font-bold" style={{ color: '#96d629' }}>{totalPiezas} piezas disponibles</span>
          </p>
        </div>
        <button
          onClick={openNew}
          ...
          <div className="grid grid-cols-3 gap-3">
            {top3.map((s, i) => (
              <div key={s.id} className="rounded-xl p-3 text-center"
                ...
                <p className="text-2xl font-black" style={{ color: i === 0 ? '#15803d' : i === 1 ? '#1d4ed8' : '#92400e' }}>
                  {s.cantidad}
                </p>
                ...
                <div key={s.id} className="flex items-center gap-3 px-5 py-3 hover:bg-zinc-50 transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-bold text-zinc-800 truncate">
                      {s._producto?.nombre || <span className="text-red-400">⚠ {s.productoId}</span>}
                    </p>
